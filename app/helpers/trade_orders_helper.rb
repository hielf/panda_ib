...
module TradeOrdersHelper
  ...
  def market_data(contract, version, force_collect=false, db_collect=ENV['db_collect'])
    ...
    begin
      ...
      PyCall.exec("bars = ib.reqHistoricalData(contract, endDateTime='', durationStr='#{duration} S', barSizeSetting='#{bar_size}', whatToShow='TRADES', formatDate=1, useRTH=False, keepUpToDate=True)")
      PyCall.exec("for i in range(len(bars)): bars[i].date = bars[i].date + timedelta(hours=8)")
