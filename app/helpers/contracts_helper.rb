...
module ContractsHelper
  ...
  def py_check_position(contract, amount = ENV["amount"])
    Rails.logger.warn "ib py_check_position start"
    order = ""
    ...
    end_time = Time.zone.now
    Rails.logger.warn "ib py_check_position run: cd #{Rails.root.to_s + '/lib/python/ib'} && python3 v4_#{ENV["backtrader_version"]}.py '#{csv}' '#{json}' '#{begin_time}' '#{end_time}'"
