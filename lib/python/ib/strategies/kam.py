...
class kam001(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position: # 没有持仓
            ...
            if self.position.size > 0:
                # 不能小于开仓价格
                if self.dataclose[0] < self.buyprice - self.datatr[0]:
                    self.log('long1:小于开仓价')
                    is_close = True
                
                # 移动平仓- 下移动平仓单
                if self.data.close_5T[0] < self.old_close5T:
                    self.log('long2:移动平仓, 最新收盘价, 比上一个5分钟价格低')
                    is_close =True
                
                #冲高回落
                if (
                    self.data.high[0] > self.datahh[0] + self.datatr[0]/2
                        and (self.dataclose[0] < self.datahh[0])
                    ):
                    self.log('long3:冲高回落')
                    is_close = True

                # 跟踪平仓, 当前价格和最高价的差
                if self.dataclose[0] < self.maxprice - 30:
            ...
            if self.position.size < 0:
                ...
                # 不能>开仓价格
                if self.dataclose[0] > self.sellprice + self.datatr[0]:
                    self.log('short1:>开仓价')
                    is_close = True
                
                # 移动平仓- 下移动平仓单
                if self.data.close_5T[0] > self.old_close5T:
                    self.log('short2:移动平仓, 最新收盘价, 比上一个5分钟价格低')
                    is_close =True
                
                #冲高回落
                if (
                    self.data.low[0] < self.datall[0] - self.datatr[0]/2
                        and (self.dataclose[0] > self.datall[0])
                    ):
                    self.log('short3:冲高回落')
                    is_close = True

                # 跟踪平仓, 当前价格和最高价的差
                if self.dataclose[0] > self.minprice + 30:
