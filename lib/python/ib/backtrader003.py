...
if __name__ == '__main__':
    ...
    dataframe = pd.read_csv('./data/hsi202011.csv', index_col=0, parse_dates=True, usecols=['date', 'open', 'high', 'low', 'close', 'volume'])
    dataframe['openinterest'] = 0
    data = bt.feeds.PandasData(dataname=dataframe,
                            fromdate = datetime.datetime(2020, 11, 2, 9, 0),
                            todate = datetime.datetime(2020, 11, 6, 17,15)
                            ) # 年月日, 小时, 分钟, 实盘就传参数吧
    ...
    with open('./data20201101w.csv', 'w') as f:
