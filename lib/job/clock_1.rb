...
module Clockwork
  ...
  handler do |job, time|
    ...
    TradersJob.perform_now 'hsi_15secs' if job == 'ib.trader'
  end

  every(1.second, 'ib.trader', :if => lambda {  |t| [0,15,30,45].include? t.sec }, :thread => true) if ENV["backtrader_version"] == "15sec"
